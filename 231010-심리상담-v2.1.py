...
def chrome_driver_path():
    ...
    time.sleep(0.5)
    # 크롬드라이버 설치된 경로로 수정
    return r"C:\Users\USER\Desktop\pthyon workspace\1_심리상담 대상자 취합-221023\24\chromedriver.exe"
...
try:
    ...
    # 로그인 시작
    driver_Happy.find_element_by_id("user_id").send_keys("hc_csj1")
    driver_Happy.find_element_by_id("pwd").send_keys("dudn1591!")
    ...
    # 지역이동 시작
    loc_Address = pd.read_excel("경기도.xlsx")
    대구분 = ""
    중구분 = ""
    소구분 = ""
    ...
    # 데이터 분류 시작
    print("데이터 분류 시작")
    data_Extract = pd.DataFrame(columns=['사용일', 'TID', '우울감', '고독감', '대화내용'], dtype=str)
    ...
    for f_num in range(file_Number):
        if f_num >= 0:
            try:
                ...
                # 엑셀 파일 로드
                data_Origin = pd.read_excel(os.path.join(files_Path, recent_file_name))
                ...
                if 'TID' in data_Origin.columns:
                    for i in range(len(data_Origin['TID'])):
                        ...
                        if DIP >= 1 or LON >= 1:
                            data = {'사용일': DATE, 'TID': TID, '우울감': DIP, '고독감': LON, '대화내용': ""}
                            data_Extract = data_Extract.append(data, ignore_index=True)
                else:
                ...
                continue  # 다른 예외가 발생해도 다음 루프로 건너뜁니다
        else:
            break
    ...
    # 헤더 설정
    session_Header = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36'}
    ...
    for x in range(len(data_Extract['TID'])):
        ...
        # RCP정보 획득    
        result_RCP = session.post(url=url_RCP, headers=session_Header, data=payload_RCP).json()
        row_RCP = result_RCP['list']

        # row_RCP가 비어있는지 확인
        if not row_RCP:
            print(f"Warning: TID {TID}에 대한 RCP 정보를 찾을 수 없습니다.")
            continue
        ...
        # 발화어 정보 획득 페이로드
        payload_Speak = {'rcp_id': data_RCP, 'rcp_name': data_Name, 'search_date_start': DATE, 'search_date_end': DATE}

        # 발화어 정보 획득
        result_Speak = session.post(url=url_Speak, headers=session_Header, data=payload_Speak).json()
        ...
        data_Extract.at[x, '대화내용'] = data_Speak
    ...
    # 데이터를 엑셀 파일로 저장
    output_folder = r"C:\Users\USER\Desktop\pthyon workspace\1_심리상담 대상자 취합-221023\24\심리상담대상자 추출자료"
    output_file_path = os.path.join(output_folder, '{}-{}-{}~{}-{}-{} 심리상담 필요대상자.xlsx'.format(start_Year, start_Month, start_Day, end_Year, end_Month, end_Day))
